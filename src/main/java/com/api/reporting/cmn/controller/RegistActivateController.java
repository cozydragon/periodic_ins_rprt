...
import com.api.reporting.cmn.util.LogWrapper;
import com.api.reporting.cmn.util.MakeJsonResult;
...
public class RegistActivateController {
	...
	public String moduleserialsum_report(HttpSession session, WebRequest request, Model model,@ModelAttribute("cri") SearchCriteria cri) {
		...
		try{
			...
			model.addAttribute("thisyear",String.valueOf(thisyear)+"��");
			model.addAttribute("lastyear",String.valueOf(lastyear)+"��");
			model.addAttribute("twoyearsago",String.valueOf(twoyearsago)+"��");
			model.addAttribute("threeyearsover",String.valueOf(twoyearsago)+"������");
			model.addAttribute("code_regist",utilService.show_code_length("tb_regist_status","code","code_message",100));
